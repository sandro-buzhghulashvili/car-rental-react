    ...
    <section className={classes.main}>
      ...
      {showMap &&
        ReactDOM.createPortal(
          <Modal showModal={showMap} onCloseModal={showMapHandler}>
            {ctx.user ? (
              <ChooseLocation onChooseLocations={showMapHandler} />
            ) : (
              <p>You should be logged in to choose locations</p>
            )}
          </Modal>,
          document.getElementById('modal')
        )}
      <div className={classes['intro-page']}>
        <div className={classes.info}>
          <h1 className={classes.yellow}>
            EASY{' '}
            <span className={classes.nestedHeading}>
              AND FAST WAY TO RENT YOUR CAR
            </span>
          </h1>
          <p>
            Welcome to our car rental website, where we provide you with an easy
            and fast way to rent your dream car. Whether you're planning a
            weekend getaway, a business trip, or simply need a temporary ride,
            we've got you covered. With our user-friendly platform, finding and
            booking the perfect vehicle is a breeze.
          </p>
        </div>
      ...
      <div className={classes.action}>
        ...
        <div className={classes.location}>
          <div className={classes['pick-up']}>
            ...
            <span onClick={showMapHandler}>
              Current location :{' '}
              {ctx.user && ctx.user.pickUp
                ? ctx.user.pickUp.split(',')[0].toString()
                : ''}{' '}
          ...
          <div className={classes['drop-off']}>
            ...
            <span onClick={showMapHandler}>
              Drop off location :{' '}
              {ctx.user && ctx.user.dropOff
                ? ctx.user.dropOff.split(',')[0].toString()
                : ''}{' '}
              <FontAwesomeIcon className={classes.icon} icon={faLocationPin} />{' '}
            </span>
          </div>
        </div>
      </div>
